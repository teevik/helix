...
pub struct View {
    ...
    // TODO refactor to something more general
    // likely an IndexMap<String, Vec<Overlay>>
    // so layers are both accessable by name but retain their order
    /// virtual text overlay generated by the jump mode
    pub jump_labels: Rc<[Overlay]>,
}
...
impl View {
    ...
    pub fn text_annotations(&self, doc: &Document, theme: Option<&Theme>) -> TextAnnotations {
        ...
        if !self.jump_labels.is_empty() {
            let highlight =
                theme.and_then(|theme| Some(Highlight(theme.find_scope_index("ui.virtual.jump")?)));
            text_annot.add_overlay(self.jump_labels.clone(), highlight);
        }
        text_annot
    }
