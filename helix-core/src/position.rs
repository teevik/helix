...
/// to ensure consistent performs for large lines (currently unimpelementes)
...
///
/// If the targetted visual line is entirely covered by virtual text the last
...
///
/// The returned `char_idx` is garunteed to be on the same visual line as `anchor`.
/// For example if the grapheme at `anchor` is the only grapheme in the given visual line
/// this function will always return `anchor` no mater what value is passed for `column`.
///
/// If the `line` coordinate is beyond the end of the file, the EOF
/// position will be returned.
///
