...
macro_rules! register_hook {
    ...
    (move |$event:ident: &mut $event_ty: ident<$($lt: lifetime),*>| $body: expr) => {
        ...
        unsafe {
            ...
            // prove soudness. The hook must therefore accept any compination
            // of lifetimes. In other words fn(&'_ mut Event<'_, '_>) is ok
            // but examples like fn(&'_ mut Event<'_, 'static>) or fn<'a>(&'a
            // mut Event<'a, 'a>) are not. To make this save we use a macro to
            // forbid the user from specifying lifetimes manually (all liftimes
            ...
            // "accidentially" creating these problems. For example:
            //
            ...
            // event has one lifetime argument. Similar logic applys to using
            ...
            // very theoretical missuse. This won't comeup in real code (and is
