...
struct SignatureHelpModeHook(Sender<SignatureHelpEvent>);
impl Hook for SignatureHelpModeHook {
    ...
    fn run(
        ...
        match (old_mode, new_mode) {
            (Mode::Insert, _) => {
                send_blocking(&self.0, SignatureHelpEvent::Cancel);
            ...
            (_, Mode::Insert) => {
                if cx.editor.config().lsp.auto_signature_help {
                    send_blocking(&self.0, SignatureHelpEvent::Trigger);
                }
            }
            _ => (),
        }
...
struct SignatureHelpPostInsertHook(Sender<SignatureHelpEvent>);
impl Hook for SignatureHelpPostInsertHook {
    type Event<'a> = PostInsertChar<'a, 'a>;
    fn run(&self, PostInsertChar { cx, .. }: &mut PostInsertChar<'_, '_>) -> anyhow::Result<()> {
        if !cx.editor.config().lsp.auto_signature_help {
            return Ok(());
        }
...
struct SignatureHelpDocDidChangeHook(Sender<SignatureHelpEvent>);
impl Hook for SignatureHelpDocDidChangeHook {
    type Event<'a> = DocumentDidChange<'a>;
    fn run(&self, DocumentDidChange { doc, .. }: &mut DocumentDidChange<'_>) -> anyhow::Result<()> {
        if doc.config.load().lsp.auto_signature_help {
            send_blocking(&self.0, SignatureHelpEvent::ReTrigger);
        }
        Ok(())
    }
}

struct SignatureHelpSelectionDidChangeHook(Sender<SignatureHelpEvent>);
impl Hook for SignatureHelpSelectionDidChangeHook {
    type Event<'a> = SelectionDidChange<'a>;
    fn run(
        &self,
        SelectionDidChange { doc, .. }: &mut SelectionDidChange<'_>,
    ) -> anyhow::Result<()> {
        if doc.config.load().lsp.auto_signature_help {
            send_blocking(&self.0, SignatureHelpEvent::ReTrigger);
        }
...
pub(super) fn register_hooks(handlers: &Handlers) {
    register_hook(SignatureHelpModeHook(handlers.signature_hints.clone()));
    register_hook(SignatureHelpPostInsertHook(
        handlers.signature_hints.clone(),
    ));
    register_hook(SignatureHelpDocDidChangeHook(
        handlers.signature_hints.clone(),
    ));
    register_hook(SignatureHelpSelectionDidChangeHook(
        handlers.signature_hints.clone(),
    ));
}
