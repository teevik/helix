use crate::{
    ...
    ui::{
        document::{render_document, Decoration, DecorationManager, LinePos, TextRenderer},
...
impl EditorView {
    ...
    pub fn render_view(
        ...
        if is_focused {
            let cursor_cache = decorations.add_decoration(&editor.cursor_cache);
            let cursor = doc
                .selection(view.id)
                .primary()
                .cursor(doc.text().slice(..));
            decorations.register_positon(cursor_cache, cursor);
        }
