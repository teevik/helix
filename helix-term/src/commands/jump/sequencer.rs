...
pub struct JumpSequence(Vec<&'static str>);

impl JumpSequence {
    /// Prefix the current sequence with the given character
    pub fn prefix(&mut self, c: &'static str) {
        self.0.push(c);
    }
}

impl From<&'static str> for JumpSequence {
...
pub trait JumpSequencer {
    ...
    fn choose(self, key: char) -> Option<Box<Self>>;
...
impl From<&'static str> for TrieNode {
    fn from(key: &'static str) -> Self {
        TrieNode {
            key,
            children: vec![],
...
fn make_trie_children(keys: &'static str) -> Vec<TrieNode> {
    (0..keys.len())
        .map(|i| TrieNode::from(&keys[i..i + 1]))
        .collect()
}
...
impl TrieNode {
    pub fn build(keys: &'static str, jump_targets: Vec<Range>) -> Self {
        assert!(!keys.is_empty());
        // Invalid key for the root node since it doesn't represent a key
        let mut root = TrieNode::from("");
...
//     fn next(it: &mut std::vec::IntoIter<JumpAnnotation>) -> Option<(Vec<&'static str>, usize)> {
//         match it.next() {
